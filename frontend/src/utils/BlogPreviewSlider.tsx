...
//         const oldBlogs = oldData?.pages.flatMap((page: any) => page.blogs) || [];
//         return {
//           pages: [{ blogs: [newBlog, ...oldBlogs], nextCursor: null }],
//           pageParams: [null],
//         };
//       });
//     },
...
export default function BlogPreviewSlider({ title, content, setIsOpen, isOpen, authorId }: BlogPreviewSliderProps) {
  ...
  const mutation = useMutation({
    mutationFn: updateBlog,
    onSuccess: () => {
      // Invalidate and refetch blogs to get the latest data from the server
